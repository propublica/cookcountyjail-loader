...
COPY_QUERY_TEMPLATE = """\copy inmates (
    Age_At_Booking,
    Bail_Amount,
    Booking_Date,
    Booking_Id,
...
def process(ctx, overwrite=False):
    print('process')
    for relative_path in glob.glob('data/raw/*.csv'):
        print(relative_path)
        ...
        if overwrite or not os.path.isfile(clean_path):
            ...
            with open(relative_path) as rf, open (clean_path, 'w') as wf:
                ...
                if 'Incomplete' not in fieldnames:
                    fieldnames.append('Incomplete')
...
def load(ctx):
    ...
    for filename in glob.glob('data/processed/*.csv'):
        print(filename)
        query = COPY_QUERY_TEMPLATE.format(filename=os.path.realpath(filename))
        ctx.run('psql {db_url} -c "{query}"'.format(db_url=DB_URL, query=query))


@task(drop_tables, create_tables, fetch, load)
def reset(ctx):
    pass


@task(drop_db, create_db, create_tables, fetch, load)
def bootstrap(ctx):
    pass
